...
double f(float n, float b, float s, float h) {
...
double complexAbs(struct Complex z) {
	return sqrt(z.r * z.r + z.i * z.i);
}

double getSmoothIteration(struct Complex z, struct Complex c, int maxIterations) {
	int i = 0;
    struct Complex zn = getMandelbrotComplexNumber(z, c);
...
int* getSmoothPixelValue(double s1) {
	// float h = 0.95 + 10 * s1;
	// return HSBtoRGB(h, 0.6, 1.0);
	float h = 0.5+0.7*(sin(s1 * 0.1 * 1.1 * 0.63));
	float s = h * h;
	float v = h * s;
