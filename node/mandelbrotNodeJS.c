...
EXTERN EMSCRIPTEN_KEEPALIVE uintptr_t getMandelbrotImagePixels(int width, int height, double x1, double x2,double y1, double y2) {
    ...
    // struct Complex c = {.r = 0.38, .i = 0.3 };
    // double* result = getIterations(z, c, maxIterations);
    // return (uintptr_t)result;
    double pixels[height * width * 5];
    ...
    for (i = 0; i < width; i++) {
        for (j = 0; j< height; j++) {
            ...
            // pixels[count++] = (double)i;
            // pixels[count++] = (double)j;
            // pixels[count++] = (double)iter;
            // pixels[count++] = x;
            // pixels[count++] = y;
...
int main(int argc, char** argv) {
    // int width = 50;
    // int height = 50;
    // int* p = getMandelbrotImagePixels(width, height, 0.3, 0.5, -0.3, -0.5);
    // FILE * pFile;
